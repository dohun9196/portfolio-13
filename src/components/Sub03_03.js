...
const Sub03_03 = ({ SubHeaderData, BrandNAV02, BrandContent02 }) => {
            ...
            <Section className='SubPage03_02 sec'>
                <div className="inner">
                    <div className="sp_tit pro01">
                        <h2>{BrandNAV02[0].MainTitle}</h2>
                    </div>
                    <SpNav>
                        <Ul>
                            {
                                BrandNAV02[0].NavTitle.map((it, idx) => {
                                    return (
                                        <Li key={idx}
                                            className={on === idx ? "on" : ""}
                    ...
                    <ProInfo>
                        ...
                        <TextArea>
                            <Title>{BrandContent02[on].MainTitle}</Title>
                            <Desc>{BrandContent02[on].Desc01}</Desc>
                            <Desc>{BrandContent02[on].Desc02}</Desc>
                            <TextUl>
                                {
                                    BrandContent02[on].List?.map((itm, idx) => {
                                        return (
                                            <TextLi key={idx}>
                                                {itm.List_Itm}
                                            </TextLi>
                                ...
                                <TextLiLast>{BrandContent02[on].Alter}</TextLiLast>
                            </TextUl>
                        </TextArea>

                        <SearchArea>
                            <ColTitle>{BrandContent02[2].ColorTitle}</ColTitle>
                            <ColUL>
                                {
                                    BrandContent02[2].Color?.map((col, idx) => {
                                        return (
                                            <ColList key={idx}
                            ...
                            <ColBox>
                                <StyTitle>{BrandContent02[3].StyleTitle}</StyTitle>
                                <StyUL>
                                    {
                                        BrandContent02[3].Style?.map((sty, idx) => {
                                            return (
                                                <StyList key={idx}
